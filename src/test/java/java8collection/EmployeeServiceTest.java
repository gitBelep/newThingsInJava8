...
public class EmployeeServiceTest {
    ...
    public void test_removeWhereSalaryIsLowerThan() {
        ...
        List<Employee> removed2 = employeeService.removeWhereSalaryIsLowerThan(new ArrayList<>(employees),
                170_000);
        List<String> remainingNames = removed2.stream().map(Employee::getName).collect(Collectors.toList());
        assertTrue( remainingNames.contains("John Doe"));
    ...
    public void test_updateEmployees() {
        List<Employee> changedEmployees = Arrays.asList(
                new Employee(1L, "John Doe", 250_000),
                new Employee(5L,"Jane Smith", 100_000) );

        Map<Long, Employee> updated = employeeService.updateEmployees(
                new HashMap<>(                                          //param1: Map
                employees.stream()
                        .collect(Collectors.toMap(Employee::getId, e -> e))),
                changedEmployees );                                     //param2: List
    ...
    public void test_salariesChanged() {
        List<Employee> changedEmployees = Arrays.asList(
                new Employee(1L, "John Doe", 180_001),
